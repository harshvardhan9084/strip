...
window.__qa30 = (async () => {
  ...
  // ---------- B1: mount sweep ----------
  {
    ...
    div.remove();
    ok('B1 all cartridges mount clean with the R30 affordances live', fails === 0 && total === Strip.all().length  /* R32 amendment: registry-length self-consistent — the deck grew to 52 */,
       fails + '/' + total + ' failed');
  }
