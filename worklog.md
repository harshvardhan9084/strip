...
Task ID: R35
Agent: Super Z (main)
Task: Round 35 — "The One Anatomy" P1/P2 wave: the second implementation round off auuudit.md (the 52-cartridge teardown), per the R34 worklog's own ③ roadmap (ceremony adoption sweep, verb hierarchy, single-hint rule, density wave, access settings).

Work Log:
1. **CEREMONY ADOPTION SWEEP (worklog-③ item 6) — 9 cartridges wired to the R34 run-ceremony engine** (7→16 of the scored deck): whackmole TIME UP/moles, maze REACHED/moves (inverted encoding passes a DISPLAY-READY count — the panel never sees the stored CEILING−x), memorymatch SOLVED/moves, slidepuzzle SOLVED/moves, simonsays SEQUENCE BROKEN/rounds (dying on round 1 still speaks: hero null, honest empty — the store contract "nothing reported at 0" untouched), typespeed TYPED (win) / UNDER 80% (over) with the delta explaining WHY a sub-80% run banks nothing, unscramble STREAK BROKEN (gated at dead streak ≥ 3 — a 1-word stumble isn't a run worth interrupting), flapdot DOWN/bars (death was an 11px line at the field's bottom edge; the in-card hint was removed entirely under the single-hint rule — the panel verb is the retry invitation now), balloonpop TIME UP/popped. Every restart verb's handler opens with RunCeremony.hide() — a restart never fights the flourish.
2. **VERB HIERARCHY (P1)**: blackjack shows exactly ONE verb per phase — betting = Deal alone, play swaps the row to Hit/Stand/Double (display:none, not 45%-opacity ghosts; the audit's "Deal hides among 4 equal buttons" is dead). breakout + whackmole: the mid-run "Playing…" disabled accent control is now a plain .status-chip (BALL n / ROUND LIVE) — a status is not a disabled button (new CSS class, PS2P 9px, no glow, cursor:default). unscramble Skip demoted from purple accent to plain .btn. TD's wave button verified already-accent (promoted in an earlier round).
3. **SINGLE-HINT RULE (P1)**: anthill's vault "Tap to collect into Food" line REMOVED (shell hint owns it); aquarium's idle status line goes QUIET when there's nothing to say (it only speaks on states the shell can't know: armed fish, out of food); blobmerge's 3-clause goal paragraph compressed to one glanceable line "GOAL blob 8 · off-board recycles · best N"; dicepig's log restated the shell rules — it narrates STATE now ("Turn 1 of 5 — roll."). bubbleshoot's duplicate was already gone locally. flapdot covered in (1). **Copy carried earlier rounds already fixed randomfact/thisorthat/kingdom/perfectring — the audit's P3 voice list is fully paid.**
4. **DENSITY WAVE (P1)**: aquarium — the wrap stretches the full card body and the tank owns the playfield slot (flex:1, min 230px, was a 300×150 box floating in ~45% dead card) and fish DOUBLED (18→36px base). blackjack — a face-down SHOE sits on the felt's corner at all times (the audit's "the game never visually happened"). balloonpop — pops burst (6 shards + "+1"/"−3" float, reduce-motion gated to float-only), bombs pulse a fuse glow BEFORE the tap (.bp-bomb keyframes; the global reduce-motion kill-switch covers it), and a 1.2s combo window floats COMBO ×N at 3+. breakout — field grows to min(78vw,290px) (same 240×320 coordinate space, bigger pixels; physics untouched). bubbleshoot's in-canvas reflow and TD court remain the two documented skips (both playable; low ROI vs risk this round).
5. **ACCESS SETTINGS (P2, the audit's "ship these five")**: settings sheet REGROUPED to the audit's 5-group mental model — PLAY (Sound/Interface sounds/Haptics/Haptic strength/Volume — the feel cluster reunited), SCREEN (Color scheme/NEW auto night/CRT skin/Screen glow/CRT effects), ACCESS (NEW: Text size S/M/L, NEW: Colorblind symbols, NEW: Flash reduction, Reduce motion — moved here: motion is an access control, not a toy), SESSION (Fullscreen/Nav arrows/Left-handed/Lock scroll/Keep awake/Daily nudge), YOUR DATA unchanged + Install back inside it. All keys identical; zero migration needed. **Text size** = html[data-text-size] for CSS-owned sizes + Settings.applyTextScale() DOM pass for the game-owned INLINE styles (idempotent: base pinned in data-fs0, dust zone 8–12px only, L=+2/S=−1/M restores; called on every settings change AND on card mount from app.js). **Colorblind symbols** = html.cb-symbols + [data-cb]::after CSS overlay — simonsays pads/dropfour discs/chainlink dots stamp ▲●■◆ at render, bubbleshoot draws canvas glyphs per frame (canvas can't take the class; checked per draw so the toggle flips live). **Flash reduction** = html.flash-safe caps --glow-mul to .18 (below Soft) + kills the .btn press flash — brightness, not motion. **Auto night chassis** = the clock picks the scheme 19:00–07:00 (10-min interval + visibility re-check; writes only on bucket change so applyToDocument re-entrancy is bounded); a manual scheme pick sets autoNight:false — two writers on one dial is how sheets learn to lie. **Board scale carried** (needs per-game wiring like sokoban's; documented, not faked).
6. sw v33→v34 (no new files; changed js re-verified through a fresh origin), qa/r35 NEW — see verification below.

Verification (all live via agent-browser on served builds, fresh sessions):
- **qa/r35 NEW 48/48** (fresh session, port 8778): A1 sheet structure (5 groups, 3 segs, 3 new controls, install in DATA); A2 REAL text-size roundtrip on a mounted card's inline stat rows (10→12→9→restore 10 — the first live run CAUGHT the L→M stranded-size bug, fixed: the "m" path now restores pinned bases); A3 colorblind live (pads ▲●■◆, real dropfour drop leaves ▲, chainlink 82 dots stamped, toggle-off live); A4 flash-safe glow-mul 1→.18→1 computed; A5 auto-night hour bucket applied + manual pick stands it down; A6 REAL plays: simonsays death → SEQUENCE BROKEN, typespeed completion → TYPED/wpm, flapdot death → DOWN/bars + verb retire (first run caught a 250ms-vs-260ms fade race — wait lengthened), balloonpop End round → TIME UP; A7 verb hierarchy (blackjack bet=[Deal] play=[Hit,Stand,Double], breakout BALL 3 chip, whackmole ROUND LIVE chip, unscramble Skip plain, TD accent); A8 density (shoe 3 layers, tank flexGrow 1 + fish 21.6px, bomb pulse + +1 float through a real pop); A9 single-hint pins (dicepig/blobmerge/anthill/aquarium, aquarium asserted POSITIVELY after the first run's vacuous pass); C1 52/52 sweep; C2 zero console errors.
- **Gauntlet r23–r34 all green on fresh sessions: 18/25/18/15/10/9/11/12/12/13/11/30 = 184 assertions.** Two infrastructure lessons recorded: (a) the agent-browser CLI JSON-quotes string outputs — shell comparisons must strip quotes, and suite results are a plain ARRAY (`return results`), not {results}; (b) the fresh-session wipe must complete BEFORE navigation or the new page re-opens the DB first — a wipe/navigation race contaminated r31–r34 once (8 depth/hint failures that all vanished on a strict re-run; the recorded R33 protocol lesson again, now with the failure signature).
- **Live screenshots** (download/strip-r35-shots/): ACCESS group on the dark sheet, simonsays pads with ▲●■◆, dropfour light chassis with symbols, balloonpop TIME UP ceremony on paper, blackjack felt with the shoe + Deal-only bet row, aquarium tank fill.
- `node --check` clean on all 25 touched files; fresh boot 0 errors, 52/52 registered, SW v34 installs and controls.

Stage Summary:
- HEAD moved R34 → R35: 16 of the scored cartridges now celebrate through the ONE run-ceremony panel; the verb hierarchy and single-hint rule are deck-wide disciplines; the settings sheet carries the audit's ACCESS group with four real dials (text size, colorblind symbols, flash reduction, auto night) — every one live-flipping without remounts.
- Honest scope notes: board scale (P2) carried — it needs per-game cell wiring, not a dial alone; bubbleshoot in-canvas reflow + TD court density carried; memorymatch/slidepuzzle/maze/whackmole/unscramble ceremony panels verified by engine + shared wiring shape (typespeed/flapdot/simonsays/balloonpop verified through REAL full plays in-suite) — the brute-force solve path exists in the session log as a manual verification for memorymatch (SOLVED/168/NEW GAME through ~40 real flips).
- ③ Next-phase priorities: (1) P2 meta — the sparkline footer on every scored card (exists on ~3; the component is js/sparkline.js, adoption is mechanical); (2) the shell-owned STAT ROW (the last un-raided P1 item — schedule as its own round, it touches 52 headers); (3) board scale setting (per-game cell wiring, start with sokoban's R34 scaler as the pattern); (4) real-device pass (19th carry) — now includes text-size L legibility and the ceremony panels on a real screen; (5) suite hygiene — the A0 `>= N` conversion (R32 carry) remains open, and the gauntlet harness now exists as scripts/r35_gauntlet.sh (lessons baked in: quote-stripping, array results, strict wipe-before-navigate).

---

## Round 36 — The One Anatomy, the final wave off auuudit.md

**① Current project state (assessment)**
- Baseline: HEAD was R35 (6ed9c10) with R34+R35 having paid the audit's P0 wave (ceremony engine, contrast floor, win-line strokes, toasts, type floor) and P1/P2 wave (ceremony adoption ×9, verb hierarchy, single-hint rule, density, four ACCESS dials). The worklog's ③ listed three carry items: sparkline-everywhere (P2 meta), the shell-owned STAT ROW (the last un-raided P1 item, "touches 52 headers"), and the board-scale setting. This round closed ALL THREE, which retires every P0/P1/P2 line of the audit's priority matrix (P3 was paid in R35). The local server + agent-browser QA flow is warm; two real product bugs were caught and fixed by the old suites during this round's gauntlet (details in ②).
- New since R35: the card anatomy is now LITERAL — every card ships with a shell-owned `.cart-stats` slot between title and playfield; games DECLARE stats instead of hand-rolling headers. 39 cartridges converted (grep: zero `statRow` remains). The sparkline chip now appears after a game's FIRST play (thin chip, no chart). A third ACCESS dial (BOARD SCALE S/M/L) rides `--board-scale` through calc() in 10 grid games, live, with no listeners.

**② Current goal / changes made / verification**
- Goal: implement auuudit.md's remaining priorities — the ONE stat row, the readout on every scored card, board scale — and pin them with a new regression suite + full gauntlet.
- SHELL (js/app.js): `buildCardShell` now emits `<div class="cart-stats" hidden>` between header and body. `makeApi(entry)` (was `makeApi(id)`) grew `setStats([{label,value,color}])`: ≤4 cap, signature-idempotent (same data never touches the DOM), empty list hides, first-party color guard against style injection, values/labels escaped. Slot is reaped by the SHELL on unmount AND on remount (stale SCORE never outlives its session). A `strip:gameover` listener busts the centered card's sparkline TTL cache so a first-ever play renders its chip immediately.
- CSS: `.cart-stats` component (13px values / 11px labels, display font, tabular nums, text-size hooks 12/15px) + `--board-scale` var engine (S .82 / M 1 / L 1.22 via `html[data-board-scale]`).
- STAT ROW adoption — 39 cartridges converted (two codemods for the two dominant patterns, then 24 by hand for compound/status-line cases): anthill, aquarium, artillery, balloonpop, blackjack-area checks, blobmerge, blockfall, breakout, bubbleshoot, chainlink, codebreaker, colormix, colorsnap, dicepig, dropfour, flapdot, garden, kingdom, lightsout, maze, memorymatch, minisudoku, minesweeper, perfectring, plinko, pongduel, rhythmtap, simonsays, sandbox-family checks, snake, sokoban, stacktower, towerdefense, tradingpost, trivia, typespeed, unscramble, whackmole, xox. Every ≤4-stat deck rule honored: 5-stat headers (kingdom DAY/GOLD/FOOD/POP/BEST; aquarium; artillery) drop BEST — which the now-always-on sparkline chip carries. Moments, not stats, stayed in-card: pongduel's MATCH POINT pulse, blobmerge's COMBO chain chip. Words moved to dedicated message lines (not stats): codebreaker's CODE WAS reveal, lexicle's rejections/answer reveal, sokoban's daily instructions + CRATE SHIPPED lines.
- CEREMONY adopters added while converting (the audit's "win moments are silent" tail): minisudoku SOLVED (tone clear, honest delta, NEW PUZZLE verb) and dropfour YOU WIN/AI WINS/STALEMATE (streak as the hero number); both restart verbs open with RunCeremony.hide(). minisudoku's in-card instruction sentence was deleted under the single-hint rule (the shell hint already says it).
- SPARKLINE EVERYWHERE (js/sparkline.js): a record with one play renders the thin chip — BEST (+ FEWER WINS for inverted encodings) + DEPTH readout, no chart; two plays upgrade to the charted chip; zero data still removes the chip (absence stays honest).
- BOARD SCALE: settings key `boardScale` + sheet segment (ACCESS group, bscale-seg-btn, syncs like its siblings). Wiring: 8 DOM grids via inline `calc(px * var(--board-scale,1))` (minesweeper, minisudoku, lightsout, slidepuzzle, xox, dropfour, whackmole, memorymatch — live re-grade, zero listeners); chainlink via wrap calc (its svg viewBox scales itself); sokoban via numeric cell-math × scale + a Settings.onChange re-render (listener unsubscribed in cleanup). artillery hardened: draw() refuses to paint an unmeasured canvas (the NaN-gradient race the gauntlet caught).
- REAL BUGS the round caught (both fixed): (1) tradingpost armed its drift/autosave intervals BEFORE the first render — any mount-time throw would leak armed timers that outlive the card (the gauntlet watched it spam `api.setStats is not a function` uncaught, once per second, forever). First paint now happens before the timers are armed. (2) artillery could paint before fit() on a loaded page — `createLinearGradient(0,0,0,undefined)` = NaN throw; draw() now guards `W > 0 && H > 0`.
- QA: qa/r36-regression.js NEW — 32 assertions: shell slot existence + mount-time population, real lightsout tile-tap bump, no-stat toys keep the slot hidden, ≤4 cap + same-node idempotence + empty-list hide, real conversions (TD/kingdom/blockfall/dropfour-drop/minesweeper-dig), unmount reaping (deck-order-aware jump), thin→charted sparkline upgrade + gameover cache-bust, board-scale var/attr/calc re-grade + sokoban numeric re-render + sheet sync, stat-row text-size hook, 52/52 sweep, zero console errors. Suites amended for the new anatomy: r34 B1b probes the shell slot's 11px label (TD's inline header no longer exists); r35 A2 probes codebreaker's inline 10px line (whackmole's moved out); r26–r33 stub-mount api stubs grew `setStats(){}` (the shell api contract gained a method — a stub without it made mount-time setStats calls throw, which is how bug (1) was found).
- Verification: node --check clean ×52 games + shell; fresh-boot 0 errors; qa/r36 32/32 (three runs); screenshots in download/strip-r36-shots (dark + light stat rows for TD/kingdom/lightsout/minesweeper/breakout/sokoban/dicepig/trivia/minisudoku, board-scale M vs L pair, sparkline chip visible with DEPTH readout). Full gauntlet r23–r36 on fresh sessions: 274/274 assertions green (r23 18, r24 25, r25 18, r26 15, r27 10, r28 9, r29 11, r30 12, r31 12, r32 13, r33 11, r34 30, r35 48, r36 32).
- Suite-amendment ledger (the R36 shell contract rippled into the older pins, all resolved): (1) `_testMakeApi(id)` → `_testMakeApi({ mod: { id }, el })` — 18 call sites across r20/r23/r24/r25/r32/r33 migrated (the string form throws inside makeApi now, which hung r23's async suite and crashed r25's mountAndCheck — the gauntlet caught both); (2) stub-mount api stubs in r26–r33 grew `setStats(){}`; (3) r23 B1 reads trivia's BEST RUN from the shell slot (the hover-only "normalized" note retired with the inline row — it was unreachable on touch; the * marker stays); (4) r25 A7's contrast probes search host + shell slot; (5) r34 A4 reads stacktower HEIGHT from the slot; r34 B1b reads TD's 11px label from the slot; r35 A2 tracks one element across the L/S/M pass and probes codebreaker's inline 10px line (whackmole's row moved out); (6) r36 A4 jumps by absolute deck index (the deck grows past 52 with repeats — module-identity jumps can land adjacent).

**③ Open problems / risks / next-phase priorities**
- Carried: real-device pass (now 20th carry — should include the shell stat row legibility, board-scale L on a small phone, and ceremony panels); suite hygiene A0 `>= N` conversion (R32 carry, still open); bubbleshoot in-canvas reflow + TD court density (audit P1 density leftovers, deliberately carried from R35).
- Risk notes: the stub-mount sweeps in old suites mount games OUTSIDE the shell — any future shell-api addition needs the same stub update discipline (r26–r33 were amended this round; consider a shared stub module in a future hygiene pass). Games that arm timers before first paint: tradingpost was the one caught; a one-round audit of mount-order across colony/idle games (garden, anthill, kingdom) would retire the class.
- Next-phase priorities: (1) a "MY SHELF" pass — the audit's personalization ladder (favorites reorder the deck) is the biggest un-raided delight; (2) P1 density remainders (bubbleshoot aim-guide dots, TD court fill) if a quieter round is wanted; (3) cartridge #53+ onboarding already teaches the shell slot (game_template.js §2 now declares api.setStats + its launch checklist should gain a "stats declared, never hand-rolled" line); the two-codemod machinery in scripts/r36_statrow_codemod*.js remains as archaeology for how the 39-header migration was paid.

---

## Round 37 — MY SHELF + the audit's long tail (auuudit.md fully retired)

**① Current project status / assessment**
- Baseline: HEAD was R36 (f3d737b) — every P0/P1/P2 line of auuudit.md's priority matrix already retired; the R36 ③ roadmap named the remaining tail: MY SHELF (audit §2 personalization #1, "the biggest un-raided delight"), the two P1 density remainders (bubbleshoot aim guide, TD court), and the game_template onboarding note. This session resumed from a compacted context and found the R37 feature work ALREADY IN THE WORKING TREE uncommitted (the previous session crashed mid-round): app.js/drawer.js/css carried a complete MY SHELF, bubbleshoot carried the trajectory preview, TD carried court fill + muster sprites, simonsays/codebreaker carried their affordance fixes, qa/r37-regression.js + scripts/r37_gauntlet.sh existed. This session therefore VERIFIED, DEBUGGED, and FINISHED the round rather than re-writing it — and the debugging earned its keep (see ② items 7-8: one real shell bug + one test-infra trap, both found by the gauntlet).

**② Current goals / completed modifications / verification results**
- Goal: finish auuudit.md to the last carried item, pin it with the new suite, run the full gauntlet on the final build, and push for the user's real-device test.
- MY SHELF (audit §2 #1 — "promote favorites into a virtual drawer filter that reorders the DECK itself; zero new data"): the deck now LEADS EVERY FRESH PASS with the pinned cartridges. `orderShelfFirst` is a stable partition (favorites keep their sub-order, rest untouched, same-array identity on an empty set) — exposed as `StripShell._shelfOrder` so the suite pins the EXACT code path nextBatch uses (r20 rule). drawer.js stays the SINGLE WRITER of __deck_meta__: app.js reads through the new `StripDrawer.getFavorites()` getter, never the storage. Favorited cards wear a ★ chip (.cart-fav, amber, glow-scaled) built at card build AND live-synced via the existing strip:fav-changed event plus one 1500ms delayed catch-up (batch 1 is built before the drawer's async loadMeta resolves). Deliberately NO load-broadcast event: xp.js pays +5 on favorites-row GROWTH through that same event — a boot-time replay could fake or swallow the payment; live chips ride the real toggle.
- Bubbleshoot trajectory (audit §10: "the aim guide is the whole game"): `trajectoryPoints()` simulates the shot from the muzzle with the SAME rules the flying bubble obeys (side-wall reflection x=2R−x form, ceiling y≤R, hit radius R×1.8 — mirroring loop()'s math, step size only affects dot spacing). draw() lays dotted preview dots (1.7px, ink-alpha .38) + an impact ring (R×0.9, .5) — only while aiming (a flying bubble would promise a spent path; a finished board's veil shouldn't advertise the next shot).
- TD court fill (P1 density carry): the canvas grew min(82vw,260px) → min(94vw,420px) — all render/click math is fit()+setTransform based, so the logical 260×260 space is untouched; the tower-select panel width follows. Plus the audit's §45 "show sprites": between waves, the next wave MUSTERS at the spawn point — up to 8 ink-dot + green-health-bar sprites in a ring with an ×N overflow count ("7 foes · normal" says HOW MANY; the sprites say WHAT IS COMING). Green is enemy-bar-exclusive (no green towers), which the suite exploits.
- Simonsays turn banner (audit §37 "no WATCH / YOUR TURN indicator"): one anchored .status-chip morphs TAP START → WATCH… → REPEAT n (the number is the sequence LENGTH to reproduce) — the player finally knows whose beat it is.
- Codebreaker active-slot invitation (audit §12 "pulse the active row"): the NEXT EMPTY slot gets a dashed amber border + the cbInvite pulse (CSS keyframes, reduce-motion kill-switch collapses it) — one slot invites, not a whole row shouting; it advances on every real peg pick.
- r31 A0 conversion (the R32 carry, retired): `Strip.all().length === 52` → `>= 52` — cartridge #53+ can no longer break the oldest suite.
- REAL SHELL BUG the gauntlet forced (fixed in app.js): unmountCard's stats reap sat BEHIND the `if(!entry.mounted) return;` guard — a never-mounted card wearing stats (the QA seam, or any future shell feature) kept them FOREVER, because prune passes early-returned. This latent hole made r36-A4 a shuffle coin-flip: it only passed when lightsout happened to land within ±1 of a previously-jumped target (~40%/run; it went green 4+ times during R36 by luck, then tails twice here). Fix: the reap now runs BEFORE the mount guard — unconditional of mount state, idempotent (guarded on !hidden), contract-restoring ("a stale SCORE never outlives its session", R36's own words). r36-A4 amended for the fixed contract: STALE stats are written AFTER the prior jump's settle-prune (220ms) — under the fixed shell any pending prune wipes a far card's slot by design.
- TEST-INFRA TRAP (documented + fixed in the gauntlet script): python http.server sends no Cache-Control, so Chrome's heuristic freshness (10% × age-since-Last-Modified) served HOURS-old shell js to the test browser and never revalidated mid-session — my unmountCard fix was invisible to three r36 runs (the page literally fetched 34632 old bytes vs 35444 on disk). SW unregister + caches.delete do NOT help (the SW's own fetch(req) shares the HTTP cache). The cure: in-page `fetch(url, {cache:'reload'})` for the shell assets BEFORE the navigation — the entry itself is refreshed. scripts/r37_gauntlet.sh now bakes this cache-warm step into every fresh-session preamble, with the full lesson in comments.
- sw v35→v36 (shell+runtime). game_template.js gained anatomy rule #6 (declare stats via api.setStats, never hand-roll a header — r36 pins enforce it).
- Verification: qa/r37 NEW 20/20 on a fresh session — A1 stable partition ('acb' ordering + same-array identity), A2 REAL drawer-star toggle → strip:fav-changed → ★ chip appears on the built card → untoggle removes it (getFavorites reflects both states, suite leaves no favorites behind), A3 trajectory pixels present in the mid-field band the old 40px stub never reached, A4 court >300px @390vw + muster green-pixel evidence pre-wave, A5 banner morph TAP START→WATCH…→REPEAT 1 through a real Start press, A6 exactly ONE .cb-next slot, dashed + animated, advancing after a real peg pick, A7 deck ≥52, C1 52/52 center+mount sweep (retry-once for the documented rAF-stall class), C2 zero console errors. FULL GAUNTLET r23–r37 on fresh sessions with the cache-warm preamble: **284/284 assertions green** (r23 18, r24 25, r25 18, r26 15, r27 10, r28 9, r29 11, r30 12, r31 12, r32 13, r33 11, r34 30, r35 48, r36 32, r37 20) — r28 needed one re-run (background-tab rAF stall, the documented class; green on retry). node --check clean across all touched files; fresh boot after a FULL wipe (IDB + localStorage + SW unregister + caches) = 0 console errors, 52/52 registered, SW v36 controlling, both R37 seams present. Live screenshots: download/strip-r37-shots/ (dicepig ★ chip on the card, bubbleshoot dotted trajectory + impact ring, TD court fill + spawn muster).

**③ Unresolved issues / risks / next-phase priorities**
- Carried: real-device pass (21st carry) — now includes the MY SHELF ★ chip on real hardware, the trajectory dots on a real touch screen (drag-aim legibility), and the TD court at 420px on a narrow phone. This round's push is the one the user will real-test.
- auuudit.md ledger: PAID IN FULL through P3 — P0 contrast floor + ceremony (R34), P1 structure/density (R35+R36+R37: the density remainders bubbleshoot/TD are DONE this round), P2 access + meta (R35/R36), P3 voice (R35), personalization #1 (R37). What remains of the audit is its OPTIONAL personalization ladder #2-#6 (deck drag-order under __deck_meta__, per-cartridge skin override from the ⋯ menu, FOR YOU recency/depth sort, profile ring hue = tier, two local profiles) — delights, not debts.
- Risk notes: (1) the HTTP-cache trap generalizes — any future hand-opened test page can silently run hours-old code; always test through the gauntlet script (cache-warm baked in) or a cache-reload sweep after edits. (2) Batch-running suites back-to-back occasionally trips the background-tab rAF stall (r28 this round; r37's C1 documents the retry) — running suites singly is the reliable shape. (3) The unconditional reap means any code that writes stats to a far card must expect the next prune (≤220ms later) to wipe them — production games only write to their own centered card, so only QA seams are exposed; noted in the r36-A4 amendment.
- Next-phase priorities: (1) the personalization ladder above, one rung per round, MY SHELF's plumbing (single-writer meta, live chip sync) as the pattern; (2) cartridge #53 with the game_template's updated anatomy rules; (3) a shared stub-module for the old suites' out-of-shell mounts (the recurring stub-amendment tax, noted since R36).

---
Task ID: r38-G1
Agent: general-purpose
Task: Round 38 line-by-line review — towerdefense, sanddrag, reaction, breathe, etch, kaleidoscope

Work Log:
- towerdefense.js: renderFloats() was DEAD CODE — defined (L521) but never called, so the R19 floating gold numbers (+bounty on kills, +clearBonus on wave clear) never rendered and the floats array grew unboundedly within a run → now called in gameLoop() after render() so floaters paint on top of the freshly cleared board.
- towerdefense.js: resetGame() didn't clear floats[] — a Restart within 900ms of the previous run's kills replayed stale gold floaters on the fresh board → floats = [] added to resetGame.
- towerdefense.js: renderPanel() painted the tower name in the upgrade panel with s.color (raw CRT hex meant for CANVAS art) instead of the tower type's cssVar token — the file's own R25 comment reserves cssVar for DOM surfaces precisely so the paper chassis re-grades it; the tower-select row already used cssVar → panel now uses TOWER_TYPES[t.type].cssVar.
- sanddrag.js: reviewed clean — no leaks (window mouseup/resize removed, timer cleared), touch prevented + touch-action:none, setTransform (not scale) keeps fit() idempotent.
- reaction.js: reviewed clean — inverted encoding declared (scoreEncoding/scoreCeiling L11), state machine fully reachable, one gameover per run, timeout cleared in cleanup, all zone colors token-based.
- breathe.js: reviewed clean — loaded blob sanitized (Number.isFinite + day-string check), day-rolls at midnight on both load and cycle-complete, timeout cleared in cleanup, state fresh per mount.
- etch.js: reviewed clean — ctx.scale() after canvas.width assignment is safe (width assignment resets the context transform, so resize() is idempotent), window mouseup/resize removed in cleanup, pad hex #E9E4D8 is the deliberate physical-pad surface (strokes need it).
- kaleidoscope.js: reviewed clean — setTransform fit, mode-changed listener removed in cleanup, touch preventDefault present, save/restore balanced in drawSegmentLine.

Stage Summary:
- 6 files reviewed (1253 lines), 3 bugs fixed (all towerdefense: dead renderFloats feature + floats leak-on-restart + raw-hex DOM panel color); suspicious-left-alone: TD pathIdx clamp on sell-during-wave can mark a far-along enemy _done (instant base leak) when the reopened route is much shorter — deliberate-looking clamp, needs a nearest-index recompute (feature-grade, not a surgical fix); TD/etch/kaleidoscope/sanddrag fit() lack the rect.width guard (artillery class) but no NaN is possible here (gradient args stay 0, no throw) and a window resize self-heals; TD renderPanel upBtn disabled state can go stale on gold gains while the panel is open (refreshes on next renderPanel); etch colorBtn swatch uses raw palette hexes (dark #1a1a1a label on dark panels until first toggle — swatch-by-design); breathe never calls api.tend() at its cycle-complete save beat unlike every other saving toy (etch/kaleidoscope tend) — possible contract gap but adding it could be a design decision, left alone; window "mouseup" release-only handlers ungated on isActive match the 4-game house pattern (spinner/etch/chainlink).

---
Task ID: r38-G3
Agent: general-purpose
Task: Round 38 line-by-line review — tradingpost, bubbleshoot, thisorthat, colorsnap, lightsout, simonsays

Work Log:
- tradingpost.js: unsanitized api.load() blob (rule 10) — a single non-finite number NaN-poisons the market (Math.max(1,NaN)=NaN) and autosave makes it permanent; missing lastSeen made elapsedSec NaN → phases/prices NaN forever. Added a sanitize block (gold/cap/traders/guild/lastSeen/event/caravan/stock/wobble/phase finite-checked, history filtered to finite numbers ≤30). R36 mount-order fix (first paint before armed timers) verified intact; drift/autosave/toastTimer all cleared in cleanup; tend-lane honored (no gameover).
- bubbleshoot.js: flight loop wall reflection was velocity-only (`vx*=-1`, bubble penetrates the wall) while the R37 trajectory preview mirrors x=2R−x — flight now mirrors position exactly like the preview (hunt item: preview must match flight). aimAt() clamped every below-shooter pointer to up-RIGHT (min(-0.15, positive)) — bottom-LEFT touches now pick the vertical on the touched side. fit() gained breakout's `if(!rect.width) return;` guard so an unmeasured canvas is never zeroed. Cleanup verified (rAF/resize/mode-changed/timer all killed); gameover fires exactly once per run (win via settleBoard OR death check, over-flag guarded).
- thisorthat.js: rapid taps inside the 220ms feedback window queued a SECOND bag advance — a pair was consumed without ever being answerable (fold had the same hole). Added an advanceTimer input lock on pick+fold, assigned the real 220ms timeout handle, and the mount now returns a cleanup that clears it (previously returned NO cleanup at all). state.total was the one unsanitized counter (legacy saves rendered "PICKED undefined") — finite-checked with the existing block.
- colorsnap.js: full line-by-line pass, no defects found — state is mount-scoped and reset in start(), one gameover per run at the natural end, timer cleared in cleanup, ramp resets, pick() guarantees honest match/mismatch, no drag/keydown surfaces. Left untouched.
- lightsout.js: bests from the save blob were used raw (corrupt entry could reach `moves < best` and the BEST stat) — now rebuilt into a fresh object with finite numbers only. newGame() called checkSolved() on the freshly scrambled board, so the rare all-toggles-cancelled scramble fired the FULL win path (gameover "win" at 0 moves, setHighscore(100000), BOARD CLEARED ceremony) and then re-scrambled behind the lingering panel — replaced with an inline solved check that just re-scrambles. Solvable-by-construction scramble verified; inverted encoding + scoreCeiling declaration verified present.
- simonsays.js: the WATCH→flash→nextRound playback chain was unstoppable — unmount mid-sequence left it running FOREVER in the dead card (rounds incrementing, Feedback tones playing from an invisible card, renderStats into a reaped slot). Added a `dead` flag checked at every await/entry point (playSequence/nextRound/padTap) and the mount's first-ever cleanup sets it. Sequence growth, WATCH input locking, one-gameover-per-run (finalRound>0 guard) all verified correct.

Stage Summary:
- 6 files reviewed (~2,590 lines total: tradingpost 497, bubbleshoot 495, thisorthat 165, colorsnap 159, lightsout 173, simonsays 183 post-edit), 9 bugs fixed across 5 files (tradingpost ×1 sanitize block covering 12 unsanitized fields, bubbleshoot ×3, thisorthat ×3, lightsout ×2, simonsays ×2); colorsnap clean. node --check green on all six.
- Suspicious-left-alone: (1) bubbleshoot/colorsnap/lightsout restart fns lack the defensive RunCeremony.hide(container) that simonsays/balloonpop/dropfour carry — verified NOT a live bug (ceremony.js verb click auto-hides, and the inset:0 overlay blocks every non-verb restart path), pattern divergence only; (2) thisorthat sinceProfile.n seeds from history.length%10, so the 10-pick profile-flash cadence is mount-dependent (history caps at 12) — quirk, not a hard bug; (3) simonsays pad hexes + colorsnap Stroop hexes are hardcoded accents on DOM elements — deck-standard palette, light-mode-safe as saturated fills, matches shipped design; (4) bubbleshoot has no board-drop cadence (no row insertion) — design, not a regression; (5) thisorthat pick() saves twice per pick (once pre-advance, once post) — wasteful but honest; ShuffleBag.restore only guards order indices >= n (a negative/NaN entry from a hand-corrupted save could throw) — restore is core js, noted for a future core hardening pass, not edited.
---

---
Task ID: r38-G9
Agent: general-purpose
Task: Round 38 line-by-line review — blackjack, codebreaker, pongduel, sandbox2048, breakout

Work Log:
- codebreaker.js: bests blob from api.load() used unsanitized (garbage values poisoned `used < best` and the BEST stat) → fresh-clone + finite/positive sanitize (rule 10)
- codebreaker.js: rows-exhausted loss ended the run SILENTLY — no api.gameover (deck precedent: xox/dropfour/artillery/lexicle all fire "over" on loss) → api.gameover("over", 0) once at the natural end
- codebreaker.js: after a loss the Check verb stayed enabled-but-dead (render() early-returns on failed) → submitBtn.disabled = true in the failed branch, matching the solved path
- sandbox2048.js: WIN ceremony's Continue button inherited pointer-events:none from tileLayer — unclickable, gate could never open → pointer-events:auto on the win overlay only
- sandbox2048.js: "No moves left" veil painted one input LATE (render() ran before the hasMoves check set gameOverShown) → loss check reordered before render(), veil now paints on the losing move itself
- breakout.js: launch() via the in-card Start/"Wall won — retry" button left a game-over RunCeremony panel veiling the live game (RETRY then no-ops while running → player stuck under the veil) → RunCeremony.hide(container) at the top of launch() (house idiom, cf. dropfour/slidepuzzle)
- blackjack.js: verified clean — all state mount-scoped; settleTimer cleared in cleanup; ace soft/hard math, S17 dealer, double/3:2 payouts, deck reshuffle all correct; tend-per-deal is the documented Round 24 lane (app.js names blackjack as a no-run game); card-face hexes are fixed card stock legible on both chassis (left, see report)
- pongduel.js: verified clean — state per-mount, rAF/resize/mode-changed/timer all cleaned, one gameover per match, MATCH POINT pulse hides on endMatch, AI speed capped (no teleport)
- sandbox2048.js merge math re-verified: [2,2,2]→[4,2], [4,4,8]→[8,8], [2,2,4,4]→[4,8] — merge-once-per-move rule holds; spawn only on valid moves; win/continue + game-over single-fire correct

Stage Summary:
- 5 files reviewed (1453 lines), 6 bugs fixed across 3 files; suspicious-left-alone: (1) blackjack card-face hexes #EDEAE3/#C43B3B/#1B1B24 — deliberate fixed card stock, legible on dark AND light felt (border var(--line) keeps contrast); (2) paddle/ball tunneling on stalled frames (dt cap 3 × max speed 18px step vs 14px paddle window in breakout, 9–11px window in pongduel) — classic clone edge, a proper fix needs swept collision (bigger diff), left; (3) codebreaker winning row shows no peg dots (r===row skips pegs) — cosmetic; (4) pongduel idle canvas keeps the final hot-ball frame after a match (no redraw) — cosmetic; (5) codebreaker classic win updates bests.classic in memory without a save — value never read (store is the source), harmless. node --check clean ×5.

---
Task ID: r38-G7
Agent: general-purpose
Task: Round 38 line-by-line review — aquarium, maze, dicepig, trivia, chainlink, balloonpop

Work Log:
- dicepig: BEST/GOAL stat values were literal codemod debris strings ("' + best + '" / "' + GOAL + '") rendered verbatim in the shell stat row → now String(best)/String(GOAL).
- dicepig: api.setHighscore was never called, so the banked total never landed in the store and new-best XP never paid (BEST stuck forever) → endRun now calls setHighscore(total) guarded on total > best (setHighscore stores what it's given, so the guard prevents lowering), refreshes the BEST stat on resolve.
- dicepig: rollAnim's 55ms pip-shuffle setInterval was untracked — a mid-roll unmount left it ticking → tracked in rollIv, cleared in cleanup.
- trivia: api.gameover fired ONLY inside the new-best branch — most runs ended without ever paying the shell (rule 7: exactly one gameover per run) → moved to an unconditional single call at showResults; setHighscore/buzz stay in the genuine-beat branch.
- trivia: api.load() result used unsanitized — a non-object legacy/corrupt save would throw in strict mode on savedState.bag assignment in pickQuestion (game dead on mount) → guarded to object-or-fresh.
- trivia: two hardcoded hexes in DOM inline styles (#6FCF97 answer border, meter fill) break light mode → var(--good, #6FCF97).
- chainlink: api.gameover("over", score) fired on EVERY successful chain (rule 7: never in a loop — per-pop run-ladder/mission/depth farming) → moved to the only natural run boundary, newGame(), firing once when a board with points is replaced (blobmerge jam-bank precedent); per-chain setHighscore (max-guarded) kept as-is.
- chainlink: isAdjacent allowed DIAGONAL chain steps but GAMES.md §28 pins "diagonals don't connect" → now orthogonal-only (|dr|+|dc|===1).
- chainlink: window mousemove/mouseup/touchend handlers had no StripShell.isActive gate (house rule 5; the dragging flag alone let a non-centered copy finish a drag) → gate added to onMove/onEnd.
- chainlink: chainTitleTimer (850ms fade) not cleared in cleanup → added clearTimeout.
- balloonpop: combo/comboTimer never reset in start() — a new round inherited the previous round's combo count and a stale drain timer → combo=0 + clearTimeout(comboTimer) in start().
- aquarium: state.lastSeen unsanitized — a corrupt/legacy save with non-finite lastSeen made elapsedSec NaN, poisoning every fish's age/size permanently (NaNpx fish) → finite-check fallback to Date.now() before the offline-delta math.
- aquarium: state.food unsanitized (NaN food broke feeding/regen stat math); coins not floored/clamped ≥0 → both clamped (food 0..10, coins floor ≥0).
- aquarium: loaded fish array used as-is (hunt item: validated/clamped) — non-object entries, non-finite size/hue/x/y/dir/age, and >8 fish from a corrupt save propagated into render/growth → every fish rebuilt through the mkFish factory with per-field finite checks + clamps, list capped at MAX_FISH, empty fallback respawn.
- aquarium: armed-fish outline used hardcoded #FFB347 in DOM inline style → var(--warn, #FFB347).
- maze: no defects found requiring change — generation connectivity verified sound (DFS carve spans all even-even lattice cells; the goal cell always keeps its open spanning-tree edge; random holes only add connectivity), scoreEncoding:"inverted"+scoreCeiling declared, setHighscore(INV−moves) inline, keys gated+preventDefault+removed in cleanup, touch-action:none set, gameover("win", moves) once per run guarded by won (raw moves is the deck pattern for win-only inverted games — depth.js only samples "over" outcomes, so the ratio is poison-safe by design).

Stage Summary:
- 6 files reviewed (~1,290 lines), 15 bugs fixed across 5 files (maze clean); suspicious-left-alone: aquarium api.tend() only on food BUY (R24 chose that beat deliberately), trivia per-question api.save (bag progression = meaningful transition), trivia's 800/900ms question-gap timers uncleaned on unmount (detached-DOM benign, honest run completion), dicepig/trivia/chainlink lack RunCeremony panels (own end UI, suite-pinned shapes), chainlink board-bank gameover means a session that never presses "New board" fires no gameover (design tradeoff, flagged for the parent), maze bests values not re-validated (self-written numeric shape), untracked sub-second cosmetic timers (aquarium arm-cancel/floatText, balloonpop chip/shard fades — all guarded by isConnected or detached-node removal).

---
Task ID: r38-G2
Agent: general-purpose
Task: Round 38 line-by-line review — sokoban, garden, spinner, physicsdrop, tonepad, colormix

Work Log:
- colormix.js: checkMatch had NO win latch — a slider drag inside the d<22 threshold fires `input` per pixel and EVERY event paid the full win (state.matched+=n, api.gameover ×n, save ×n, setHighscore ×n, stacked newTarget timers) → added a per-target `solved` latch (reset in newTarget, set before side effects) so gameover fires exactly once per match.
- colormix.js: state.matched from api.load() was never sanitized (undefined → "undefined" stat, then NaN on ++) → Number.isFinite fallback to 0.
- colormix.js: newTarget reset rgb to grey but left slider THUMBS at the previous target's positions (swatch/state/UI desync; a nudge then wrote surprises into G/B) → sliderEls now reset to 128 with the mix (programmatic set fires no input, so tweakCount is untouched).
- spinner.js: onUp ran for EVERY window touchend/mouseup — scrolling the deck (or any stray release) while the wheel spun banked a record and fired api.gameover("over", peakRpm) from gestures that never touched the game → onUp now early-returns unless a face-drag was live.
- spinner.js: the simulated mouse pair after a touch re-armed dragging (onDown) and fired a SECOND gameover/record on mouseup → mousedown within 500ms of a touchend is ignored (lastTouchAt); touchend routed through a named onTouchEnd so cleanup's removeEventListener actually matches it (the old code removed onUp, not the real handler).
- sokoban.js: corrupted save wedged the mount — out-of-range/fractional/negative state.level threw inside parse()/render() (LEVELS[i] undefined) → levelIdx clamped to int ∈ [0,19] after LEVELS/PARS, unlocked clamped to [0,20], bests entries filtered to finite, daily re-cloned into a fresh {last:string, streak:finite} object (house rule 10).
- sokoban.js: two untracked post-win setTimeouts (900ms auto-advance, 700ms all-clear) fired after unmount — advancing levelIdx, api.save and api.setStats on a far card → tracked in pendT[] and cleared in cleanup.
- garden.js: compost "+3" float used hardcoded near-white #EDEAE3 — invisible on the light chassis → var(--ink).
- physicsdrop.js / tonepad.js: no defects found needing changes (tonepad's Feedback.tone(freq,0.9) is the documented raw-frequency form; both scope state per mount and clean their loops).

Stage Summary:
- 6 files reviewed (~1,700 lines), 9 bugs fixed across 4 files; suspicious-left-alone: sokoban undo() moves++ (undo penalty, plausible design), sokoban api.gameover raw moves (matches codebreaker house pattern — nothing inverted goes through setHighscore, so no scoreEncoding declaration added), sokoban daily completed after midnight counts for the new day (benign edge), garden tend-on-every-tap incl. spam (deliberate per R24), garden/few ≤1s cosmetic timers untracked (detached-DOM only), spinner rAF+paintLabel every frame while merely mounted (battery nit, toy-wide pattern), physicsdrop fit() lacks zero-size guard + no resize/rotation refit (would need new machinery; flagged for a future round).

---
Task ID: r38-G4
Agent: general-purpose
Task: Round 38 line-by-line review — artillery, minesweeper, slidepuzzle, unscramble, typespeed, memorymatch

Work Log:
- artillery.js: corrupt save's difficulty fell through the skill lookup as undefined → NaN angle/power → a projectile whose every bounds comparison is false → the shot rAF spins forever, the promise never resolves, FIRE stays dead for the duel → whitelist difficulty ("easy|medium|hard", else medium) + finite-check wins/losses/streak on load (rule 10).
- artillery.js: fire() resolves its promise on dispose, so playerFire/aiTurn resumed AFTER unmount — mutating a dead card, calling api.setStats on an unmounted entry (resurrecting reaped stats, the exact thing R37's unconditional reap fights), and arming a fresh aiTimer post-cleanup → `if(disposed) return;` after both awaits.
- artillery.js: updateStat used `playerHP || 3` — 0 is falsy, so any FINISHED duel showed full ♥♥♥ in the stat row while the canvas pips showed 0 → `?? 3` (the pre-duel undefined case still renders 3).
- minesweeper.js: Android fires contextmenu ~500ms into a long-press, AFTER the 320ms pointerdown timer already flagged — the handler's unconditional toggleFlag flipped the flag straight back OFF, so long-press flagging never stuck on the primary mobile platform → contextmenu now toggles only when the long-press timer hasn't fired (longPress still truthy); desktop quick right-click unchanged, desktop held right-click no longer double-toggles either.
- minesweeper.js: a LOSS never called api.gameover at all (rule 7 — a detonation is the run's natural end; snake/breakout report losses) → one api.gameover("over", 0) in dig's and chord's mine branches; over=true makes double-fire impossible.
- minesweeper.js: fractional saved diff indexed DIFFS[1.5] → D undefined → `.W` throws at mount → Math.floor clamp.
- minesweeper.js: mid-game restore validated only the mines array — a truncated/foreign save restored revealed/flagged as undefined → render() TypeError, or a string elapsed made the clock tick NaN → require same-length revealed/flagged arrays + Number.isFinite elapsed/flags.
- slidepuzzle.js: fractional saved size indexed SIZES[1.5] → `.n` on undefined → mount throw → Math.floor clamp.
- memorymatch.js: the 700ms mismatch flip-back timer was untracked — size-pill switch or New game mid-lock rebuilt the deck, the stale timeout then indexed cards[a] on a SHORTER array → `undefined.flipped` uncaught TypeError (4×3=12 vs 6×5=30) → flipTimer tracked, cleared at newGame top, and mount now returns a cleanup that clears it (it returned NO cleanup at all before).
- memorymatch.js: same fractional size clamp as slidepuzzle.
- unscramble.js: api.gameover("over", streak) fired on EVERY solved word — XP run-ladder paid per word, the hunt-list "never in a loop / exactly once per run" class; the shell's own contract doc pins "over = … streak broken" → removed the per-solve call; gameover now fires exactly once per run when the streak dies (dead > 0) with the dead streak as score. Per-solve setHighscore kept (the real new-best path, XP-guarded by the shell).
- unscramble.js: the 400/500ms word-transition timeouts were untracked — Skip inside the solve window double-advanced (two new words), and Clear inside the wrong-answer window both got wiped mid-retry by the stale timer AND re-opened a just-solved word (chosen=[] re-enables all letters → instant re-solve streak farm) → resetTimer tracked + cleared in newWord; Clear is inert while a transition is pending.
- typespeed.js: seconds could be 0 (whole phrase landing in one input event via autofill) → division by zero → wpm Infinity → setHighscore(Infinity) → `seconds > 0` guard (wpm 0).
- typespeed.js: input.autocorrect="off" joins the existing paste/drop/contextmenu honesty guards — iOS word replacement mutates the input mid-run (false mistakes + multi-char insertions the keystroke judge can't score).

Stage Summary:
- 6 files reviewed (1,590 lines), 13 bugs fixed; node --check clean ×6. Suspicious-left-alone: artillery newDuel() measures the canvas once at mount — a 0-width mount would bake W=0 until "Next duel" (the R36 draw() unmeasured guard holds and gauntlets are green, so the mount timing was left untouched); artillery AI bracketing treats an offscreen shot's x as its landing point (harmless at these wind ranges); typespeed multi-char insertions other than autocorrect (select-all retype) are judged on their last char only; minesweeper's clearBanner rgba(10,10,16,.85)+#6FCF97 sit on their own dark scrim on both chassis (self-consistent); gameover("win", rawSeconds/rawMoves) in minesweeper/slidepuzzle/memorymatch passes the honest human number as the run score while the highscore STORE is correctly inverted+declared — matched sibling shape, left as-is; artillery's fireBtn does a disabled→text→re-enable dance in endRound (redundant but harmless).

---
Task ID: r38-G6
Agent: general-purpose
Task: Round 38 line-by-line review — anthill, whackmole, kingdom, snake, wouldyourather, plinko

Work Log:
- anthill.js: loaded colony save was never sanitized (rule 10) — a NaN lastSeen poisoned elapsedSec→vault→food→every stat; fertility could load corrupt (negative rate) → added finite/fallback sanitize for food/lifetimeFood/vault/role counts/lastSeen + fertility recomputed from farmers (its only writer); legit saves untouched.
- kingdom.js: off-by-one in the non-collapse highscore — stored state.day (days survived + 1) while the collapse path stores day−1 and the tombstone counts day−1 days; storage's setHighscore takes max() so the inflated value stuck forever → now best = day − 1.
- kingdom.js: tombstone rendered before setHighscore resolved, so a record-beating run's "best reign" understated itself → best = Math.max(best, daysSurvived) before showRecap().
- kingdom.js: loaded save numbers unsanitized — NaN gold made every build button clickable (NaN < cost is false), NaN population broke the housing cap and never collapsed, NaN assign counts poisoned idleWorkers → finite/fallback sanitize for day/gold/food/pop/farms/mines/houses/assign/peakPop before the migration clamp.
- snake.js: double-tap on "Play again" armed a second setInterval(step) → snake ran at 2× speed until the next food → clearInterval(tickId) at the top of the start handler. Input queue verified safe (setDir validates against committed dir — two quick turns can never U-turn; only a legal rapid S-turn is dropped).
- wouldyourather.js: api.load() result used unsanitized — a non-object record or any save without `votes` threw TypeError in render() and killed the mount → non-object load falls back to fresh default, votes must be a non-array object else fresh {}.
- wouldyourather.js: "your side" result bar hardcoded rgba(255,179,71,.18), deaf to the accent/theme system → rgba(var(--glow-rgb),.18) (pixel-identical on the default theme, theme-aware otherwise).
- plinko.js: peg tunneling — single-step integration with dtF clamped at 3 let a fast ball (~12px/frame, 36px in a jank spike) jump the 18px peg collision zone → adaptive sub-stepping (≤6px per sub-step, peg+wall checks per sub-step; sub=1 reproduces the old physics exactly for slow balls).
- plinko.js: fit() had no unmeasured-canvas guard (the R36 artillery-class NaN race) — a 0-width rect built NaN peg geometry and a drop could instantly settle for 100 → `if(!rect.width) return;` + `if(!cw) return;` in drop() (no NaN zombie balls).
- plinko.js: loaded state unsanitized (non-object save, non-finite drops/totalScore poison the stat row/totals) → object guard + finite fallbacks.
- whackmole.js: reviewed clean — no changes. All state mount-scoped; round timer exact; double-tap-on-mole guarded by active.has; all mole/interval timers cleared on end/start/cleanup; exactly one gameover per round; restart can't double-arm intervals.

Stage Summary:
- 6 files reviewed (~1,519 lines), 5 files edited, 10 bugs fixed (anthill 1, kingdom 3, snake 1, wouldyourather 2, plinko 3); suspicious-left-alone: anthill hill-button hardcoded brown gradient hexes (thematic earth illustration, reads fine in light mode) + no tend() on vault collect (hill tap/purchases tend; may be deliberate), whackmole banner rgba(10,10,16,.85) dark chip (readable both modes) + bannerTimer/💥 timeout not in cleanup (detached-node writes, harmless), wouldyourather rgba(255,255,255,.05) non-yours wash (near-invisible in light mode; % text carries the info) + no tend() (sibling toy thisorthat has none), plinko 600ms settle-removal timeout + idle rAF redraw (both harmless, rAF canceled in cleanup), bare Feedback calls deck-wide (house-accepted per breakout.js calibration). node --check green ×6. No core-file changes needed.

---
Task ID: r38-G8
Agent: general-purpose
Task: Round 38 line-by-line review — lexicle, minisudoku, dropfour, randomfact, stacktower, xox

Work Log:
- minisudoku.js: 6×6 generator produced INVALID base grids — the row-shift formula used S.bh (box HEIGHT) instead of S.bw (box WIDTH), so every 6×6 box held a duplicate digit (e.g. box(0,0) = {1,2,3}∪{3,4,5}); the dealt "solution" could never pass the rule-based win check → fixed to S.bw * (r % S.bh) (4×4 unchanged, bh===bw hid the bug); verified 4×4 AND 6×6 base grids now valid by node check.
- minisudoku.js: load sanitization — saved.size could be a non-integer finite (float index → SIZES[1.5] = undefined crash) → Math.floor inside the clamp; bests not re-cloned/validated (truthy garbage like a string silently dropped writes; non-finite values poisoned `seconds < best`) → object-type guard + non-finite keys deleted.
- dropfour.js: a STALEMATE fired NO api.gameover at all — a natural run end with zero ceremony/XP/depth (xox pays draws) → added api.gameover("over", 0) in the winner===0 branch (one call per run preserved).
- dropfour.js: saved.difficulty accepted any truthy garbage string → whitelisted easy/normal/hard (garbage now falls back to "normal" instead of silently behaving as normal via the aiMove else-chain).
- xox.js: Object.assign(state, saved) spread a saved blob over the template unsanitized (template rule 10; string wins/losses/draws corrupt the tally, garbage difficulty) → finite-checked the three counters + difficulty whitelisted.
- lexicle.js: stored.daily loaded unsanitized — a non-finite daily.streak string-concated on the win path ("5"+1="51") and garbage last broke the once-per-day chain → fresh factory with typeof/finite per-field restore.
- lexicle.js: Enter/Backspace/letter keys consumed without e.preventDefault() (house rule 6 — Backspace navigates history, Enter submits) → preventDefault added to each handled branch.
- lexicle.js: two untracked setTimeout classes (flashRow's 250ms border reverts ×6, the 1200ms "not in word list" statUpdate) survived unmount — the late statUpdate re-populates the shell's REAPED stat slot on a far card (the exact stale-score leak the shell reaps against) → tracked in a `tos` array via a `later()` helper, cleared in cleanup.
- lexicle.js: absent-letter tile/keyboard gray #33333D broke light mode (daylight ink #2A2620 on #33333D ≈ 1.2:1 — invisible) → var(--screen-line, #33333D): dark mode pixel-identical via fallback, light resolves to the #D9D2C4 daylight gray (9.9:1). Green/yellow KEPT as raw hexes deliberately: they are saturated game pieces (R33 keeps piece chroma) and both pass 4.3:1 with daylight ink; swapping to --good/--warn would repaint dark mode (forbidden).
- stacktower.js: perfectStreak never reset between rounds — a run ended mid-streak carried it into the next run (2 perfects in run 1 → the FIRST perfect of run 2 paid +width at ×3) → reset to 0 in reset() (all round starts go through it).
- stacktower.js: idle base block drawn off-center on every mount — reset() ran against the 300px default canvas backing store before first layout (w=300/dpr instead of the CSS min(70vw,240px)) → post-fit rAF re-runs reset() when pristine (!running && 1 block && score 0); mid-run/post-death states guarded out.
- stacktower.js: fit() sized against an unmeasured canvas (rect.width 0 → zeroed backing store; the exact race the R36 artillery hardening paid for) → breakout-style `if(!rect.width) return` guard added.
- stacktower.js: two untracked timers in cleanup — the 120ms spawn setTimeout (fires after unmount, mutating dead-closure state) and the 800ms PERFECT banner fade → spawnTimer tracked + both clearTimeout'd in cleanup.
- randomfact.js: state.favs loaded without range validation — a save from an older build (or corrupt blob) with out-of-range indices rendered "undefined" as a fact card and broke the favorites dealer → filtered to Number.isInteger && 0 ≤ i < FACTS.length at mount.

Stage Summary:
- 6 files reviewed (1603 lines read line-by-line + full re-reads of game_template/snake/breakout/ceremony/shell setStats + css tokens for calibration), 13 fixes across 6 files; suspicious-left-alone: minisudoku hole-digging does NOT guarantee unique solutions (no solver; harmless because the win check is rule-based so every legal completion wins — a solver is a feature, not a surgical fix), lexicle line ~195 comment says "abandoned games don't count" while the code counts abandoned games WITH ≥1 submitted row (ambiguous intent, code plausible), lexicle free-mode `streak` tracked+saved but never displayed (stat always shows daily streak — vestige, not defect), minisudoku TIME stat shows 0s until solved (battery-honest design; ceremony carries the time), minisudoku exact-tie with old best shows "NEW BEST" (cosmetic), stacktower rAF redraws a static frame while idle (tiny canvas, cleanup is correct), xox win-row tint is a fixed rgba(255,179,71,.15) DOM constant (translucent accent, legible in light mode). Core-file note (NOT touched, outside my list): js/shufflebag.js restore() rejects order indices ≥ n but ACCEPTS negative/non-integer ones from a corrupt save (FACTS[-5] → undefined) — randomfact/trivia/wouldyourather/thisorthat all inherit it; a one-line `typeof i === "number" && i >= 0` in restore() would close the class. node --check clean on all six files.

---
Task ID: r38-G5
Agent: general-purpose
Task: Round 38 line-by-line review — blobmerge, blockfall, perfectring, rhythmtap, flapdot

Work Log:
- blobmerge.js: WIN off-by-one — MEGA BLOB banner + winShown fired at newStage === WIN_STAGE+1 (displayed blob 9) while the goal line promises "GOAL blob 8" (displayed = stage+1, WIN_STAGE=7) and the milestone buzz already fired "win" at 8 → both checks now `newStage === WIN_STAGE`.
- blobmerge.js: jam reshuffle setTimeout was untracked → scrolling away during the 1.4s banner left a stale closure firing newGame()+persistState() AFTER unmount, clobbering the fresh session's save; also clicking "New game" mid-jam double-reset (pending timer wiped the fresh board). jamTimer now tracked, cleared in newGame() and in cleanup.
- blockfall.js: softDrop credited +1 per press even when the piece was blocked (no descent — score inflation on floor-mashing); now `if(!collides(...)) score += 1` (still credited before gravity so a game-ending drop reaches the save).
- flapdot.js: first pipe spawned in reset() BEFORE the deferred rAF fitCanvas — geometry computed against the canvas DEFAULT 300×150 (h=75 on dpr-2 phones) → gapY range [-55,40] → unavoidable/near-unavoidable death on pipe one, plus a 300×150 first-frame draw. reset() no longer spawns; the loop's spawn rule grew `!pipes.length ||` so the first bar always spawns with measured geometry (idle screen unchanged — the old pipe sat off-screen at x=w).
- flapdot.js: fitCanvas could stamp a 0×0 backing store if measured before layout (and would stay 0×0 until a window resize); added breakout's `if(!rect.width) return;` guard.
- rhythmtap.js: ring descent was per-frame (`r -= speed`) with NO dt normalization — 120Hz phones got double-speed rings and half-width tap windows (every other motion game in the deck is dt-normalized); loop(ts) now uses the flapdot dtF pattern (clamped 0..3 frame-units). 60Hz behavior pixel-identical.
- rhythmtap.js: ring strokes were `setAttribute("stroke","var(--amber)")` — var() is not reliably parsed inside SVG presentation attributes (the deck's other SVG game, chainlink, resolves colors; inline style is the canonical var() path) → strokes moved to `style.stroke = "var(--...)"`, keeping the tokens (light-mode re-grade intact, behavior-identical where attributes did resolve).
- perfectring.js: dot + live score drawn with hardcoded #EDEAE3 directly on the transparent canvas over the PANEL — invisible on the light chassis (--panel #FAF8F3; R33 daylight class). Now resolves `--screen-ink` via getComputedStyle with the raw hex as dark-mode fallback (dark pixel-identical), refreshed on strip:mode-changed, listener removed in cleanup.
- perfectring.js: keyboard gate was unguarded `StripShell.isActive(cv)` (ReferenceError in odd embeds); now `window.StripShell && ...` like every sibling.
- perfectring.js: fixed 240px backing store CSS-upscaled to 78vw → visibly soft on hidpi (hunt class "missing devicePixelRatio scaling"); added breakout's fit() (dpr-capped 2.5, setTransform to the 240 logical space) + rAF first-fit + debounced resize listener, all unwound in cleanup.

Stage Summary:
- 5 files reviewed (~1553 lines), 10 bugs fixed; suspicious-left-alone: blockfall banner/kick cosmetic timeouts not cleared in cleanup (opacity/transform on detached nodes only — no state clobber); rhythmtap 400/700ms feedback timers untracked (stale wipe can truncate the next "PERFECT" text — cosmetic; respawn timer touches only dead closure state after unmount); rhythmtap `speed` carries across a missed streak (may be deliberate pressure design); blobmerge `api.save({maxStageEver})` writes a partial blob momentarily before the full persistState in the same task (last-write-wins, harmless); blobmerge restoreState lacks an upper clamp on loaded stage (corrupt save → huge pts; JSON round-trips Infinity→null safely); flapdot spawnPipe's `h-80-gap` range unclamped (only degenerate <170px-tall viewports); blockfall rotation has no floor-kick (classic-accurate); perfectring cleanup saves an abandoned run's score as best (matches its live setHighscore mid-run habit).

---

## Round 38 — The Full Read: every line of all 52 cartridges reviewed, 88 bugs fixed

**① Current project status / assessment**
- Baseline: HEAD was R37 (6e98b0b), tree clean, auuudit.md paid in full. The user then REAL-TESTED on a device and reported: "Most of the still contains some code errors. So read every line of all games one by one and fix hand to hand. Do it on all 52 games patiently." That instruction IS this round: not a feature round, a correctness sweep over the entire deck.
- Method: a mounted-poke smoke sweep (qa/r38-sweep.js + scripts/r38_sweep.sh) first proved no game THROWS on mount/pointer/keys/light-toggle (52/52 clean, dark+light) — so the reported errors were behavioral: wrong logic, stale state, leaks, invisible-in-light surfaces. The deck's 13,774 lines of game code were then read line-by-line in 9 parallel review batches (r38-G1..G9, their per-batch detail sections are appended just above this one), each calibrated on game_template.js's ten house rules before touching its files. 12 games came back clean (sanddrag, reaction, breathe, etch, kaleidoscope, tonepad, physicsdrop, colorsnap, whackmole, maze, blackjack, pongduel); 40 files took surgical fixes.

**② Current goals / completed modifications / verification results**
- Goal: read every line of all 52 cartridges, fix every real defect found, verify zero regressions, push for the user's next real test.
- 88 bugs fixed across 40 game files (+610/−102). The headline catches, by class: MECHANICALLY BROKEN — minisudoku's 6×6 generator shifted rows by box-HEIGHT not box-WIDTH, so every 6×6 grid dealt an invalid solution the rule-based win check could never accept (unwinnable since it shipped; 4×4 hid it because bh===bw); sandbox2048's 2048-win Continue button was unclickable (overlay inherited pointer-events:none); flapdot's first pipe spawned against the unmeasured 300×150 default canvas — on dpr-2 phones the safe band collapsed to a ~15px sliver = near-unavoidable death on pipe one; colormix fired the FULL win path (gameover+save+setHighscore+XP) on EVERY slider input inside the threshold — one drag = dozens of gameovers (a direct XP/mission farm); spinner banked a record + fired gameover from ANY window release anywhere (even scrolling the deck); dicepig's header has rendered literal codemod debris "' + best + '" since R36 and setHighscore was NEVER wired (BEST stuck at 0 forever); minesweeper's Android long-press flagged then instantly un-flagged (contextmenu fired after the 320ms timer — flagging never stuck on the primary mobile platform); GAMEOVER CONTRACT — trivia paid only on new-best runs, codebreaker/minesweeper/dropfour paid nothing on loss, unscramble paid per word, chainlink paid per chain (all relocated to the one natural run boundary); UNMOUNT GHOSTS — simonsays' WATCH chain ran forever in dead cards, sokoban's auto-advance kept saving into far cards, artillery resumed mid-animation after dispose, blobmerge's jam timer clobbered the next session's save, memorymatch's stale flip-back crashed on deck rebuild, tradingpost/anthill/aquarium/kingdom/lexicle/xox/thisorthat/colormix/plinko/wouldyourather loaded saves with zero sanitization (one non-finite NaN-poisoned the whole state machine — kingdom's NaN gold made every button clickable since NaN<cost is false); LIGHT MODE — perfectring drew its dot+score in hardcoded #EDEAE3 on the cream panel (invisible — the game was literally unplayable on light), garden floats, aquarium armed-rings, trivia meter, lexicle tiles, wouldyourather bars, TD's upgrade panel all re-tokenized; PHYSICS/CANVAS — bubbleshoot's flight now mirrors the preview's exact wall math (preview never lies), its bottom-left touches no longer snap aim across the board, plinko got anti-tunneling sub-steps, rhythmtap got dt-normalization (120Hz phones had 2× ring speed = halved tap windows), perfectring/flapdot/plinko/stacktower/bubbleshoot got the measured-canvas guards, blockfall stopped paying soft-drop points for blocked presses, snake's double-tap Play-again ran double-speed; KINGDOM's day counter stored days-survived+1 on the non-collapse path (inflated bests stuck forever via Math.max).
- sw v36→v37 (both caches — game scripts are network-first so fixes reach installed clients, but the bump guarantees a clean real-test slate).
- Verification chain, in order: node --check ×52 clean after all edits; r38 sweep #2 = 52/52 mounted, 0 console errors, dark+light+poke, all games; FULL GAUNTLET r23–r37 on fresh sessions = **284/284 assertions green** (18/25/18/15/10/9/11/12/12/13/11/30/48/32/20) — not ONE pinned assertion broke across 88 behavioral fixes; live screenshots (download/strip-r38-shots) confirm perfectring's ring/dot/text now visible on light, dicepig's header reads BANKED 0 · GOAL 50 · BEST 0, bubbleshoot's dotted trajectory + impact ring render.
- TEST-INFRA LESSON (bitten live, now baked in): the R37 cache-warm covered SHELL assets only — a stale dicepig.js from Chrome's heuristic freshness rendered its R36 debris through the "fixed" build during verification and would have passed as green. scripts/r38_sweep.sh now cache-reloads ALL 52 game files before every sweep. The gauntlet's missing half: it serves on PORT 8779 and a half-asleep operator restarted only 8777 — three phantom TIMEOUT/NO-RESULT suites before the cause surfaced; both servers are now part of the boot checklist.

**③ Unresolved issues / risks / next-phase priorities**
- Carried: real-device pass (22nd carry) — this round's push is the one to test: minisudoku 6×6, minesweeper long-press flags on Android, sandbox2048's Continue, dicepig BEST, perfectring on light, bubbleshoot aim near walls.
- Deliberate tradeoffs to re-judge on device: chainlink banks gameover at New-board (a session that never resets fires none — chosen over per-chain XP farming); unscramble's run ends only when a streak dies; sandbox2048's loss veil now paints on the losing move itself; minisudoku hole-digging still doesn't guarantee unique solutions (rule-based win check makes every legal completion a win — a uniqueness solver is a feature, not a fix).
- Known-remaining classes (documented, not fixed): pongduel/breakout ball can tunnel a paddle on a stalled frame (needs swept-collision — a physics rewrite, flagged by two reviewers independently); physicsdrop lacks a rotation refit; sandbox2048 milestone-pop timer touches a detached node; kingdom's pre-R38 inflated bests persist harmlessly under Math.max; a possible deck-wide audit of window-release handlers (the spinner class) — all nine batches' "suspicious, left alone" lists live in their sections above.
- Next-phase priorities: (1) the user's real-device verdict on this build; (2) the auuudit personalization ladder rungs 2–6 remain delights-not-debts; (3) shared stub module for old suites' out-of-shell mounts (the recurring stub tax, now three rounds noted).
